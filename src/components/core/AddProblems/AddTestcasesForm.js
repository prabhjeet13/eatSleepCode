    ...
    <div className='flex flex-col gap-3'>
          ...
          <form onSubmit={submitHandler} className='scrollbar-track-gray-800 scrollbar-thin scrollbar-thumb-black gap-3 flex flex-col'>
                    <div className='flex flex-col gap-2'>
                        ...
                        <textarea rows = {5} cols = {5} value = {formData.input} name = 'input' id = 'input' className = 'p-2 bg-gray-700 text-white font-mono text-lg font-semibold' type = 'text' onChange={textboxchange}/>
                    </div>
                    <div className='flex flex-col gap-2 font-bold'>
                        <label className='font-mono text-lg'>Output</label>
                        <textarea rows = {5} cols = {5} value = {formData.output} name = 'output' id = 'output' type = 'text' className = ' p-2 bg-gray-700 text-white  text-lg font-semibold' onChange={textboxchange}/>
                    </div>
                    <button type='Submit' className='font-semibold font-mono p-2 border-2 bg-yellow-400 mt-2 rounded-md'>Save a TestCase</button>
            </form>
            <p onClick={nextHandle} className='cursor-pointer font-semibold font-mono p-2 border-2 bg-yellow-400 mt-2 rounded-md'>Test Cases Added Completed ?</p>
    </div>
  )
}
